...
def run_pipeline(input_path, output_dir, target_col, vif_threshold):
    ...
    featured.to_csv(output / "processed_full.csv", index=False)
    model_ready.to_csv(output / "model_ready.csv", index=False)
    descriptive_df.to_csv(output / "descriptive_stats.csv", index=False)
    monthly_boxplot_df.to_csv(output / "monthly_boxplot_stats.csv", index=False)
    normality_df.to_csv(output / "normality_core_metrics.csv", index=False)
    corr_df.to_csv(output / "correlation_core_metrics.csv")
    pearson_df.to_csv(output / "correlation_pearson_core_metrics.csv")
    spearman_df.to_csv(output / "correlation_spearman_core_metrics.csv")
    vif_before.to_csv(output / "vif_core_features.csv", index=False)
    vif_after.to_csv(output / "vif_core_features_after_filter.csv", index=False)

    with (output / "transition_breakpoint_report.json").open("w", encoding="utf-8") as f:
        json.dump(transition_report, f, ensure_ascii=False, indent=2)
    with (output / "vif_feature_selection.json").open("w", encoding="utf-8") as f:
        json.dump(vif_selection, f, ensure_ascii=False, indent=2)
    with (output / "preprocessing_summary.json").open("w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    for stale_name in ["ols_coefficients_hc3.csv", "ols_diagnostics.json"]:
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        default="./outputs",
        help="Directory for processed artifacts (default: ./outputs)",
    )
    parser.add_argument(
        "--target-col",
        default="rbd_p_ppm",
        help="Target column kept in model-ready output (default: rbd_p_ppm)",
    )
    parser.add_argument(
        "--vif-threshold",
        type=float,
        default=10.0,
        help="Severe VIF threshold for iterative feature removal (default: 10.0)",
    )
